...
static int freq_colloid_map        = 100000000;
...
int init_control(pe_t * pe, rt_t * rt) {
  ...
  rt_int_parameter(rt, "freq_colloid_map", &freq_colloid_map);
...
/*****************************************************************************
 *
 *  is_colloid_map_output_step
 *
 *****************************************************************************/

int is_colloid_map_output_step() {
  physics_t * phys = NULL;
  physics_ref(&phys);
  return ((physics_control_timestep(phys) % freq_colloid_map) == 0);
}
